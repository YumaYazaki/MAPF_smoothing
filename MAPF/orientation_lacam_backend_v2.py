...
class OrientationLaCAMBackendV2(LaCAMBackend):
    """姿勢付き MAPF 向け LaCAM 骨格 backend の v2。

    v2 の改善点:
        - 1 ノードあたり複数 joint successor を生成する
        - WAIT を非 goal 状態で抑制する
        - no-successor 時に blocking conflict を使って分岐する
        - 直前 config への戻りを軽く抑制する
    """
    ...
    def _low_level_expand(
        ...
        return self._construct_joint_successors(
            problem=problem,
            curr_config=curr_config,
    ...
    def _construct_joint_successors(
        ...
        def dfs(index: int, cumulative_score: float) -> None:
            ...
            if len(collected) >= self.max_joint_successors:
                return
        ...
        return LowLevelExpandResult(
            successors=tuple(unique[: self.max_joint_successors]),
    ...
    def _branch_when_no_successor(
        ...
        v2 では、各 robot の上位候補をいくつか禁止する child を作る。
        """
        del prev_config
    ...
    def _extend_node(
        ...
        # successor ranking の差を少しだけ優先度に反映する。
