...
class OrientationLaCAMBackendV4(LaCAMBackend):
    """姿勢付き MAPF 向け LaCAM 骨格 backend の v4。

    v4 の改善点:
        - v3 の単一エージェント fast path / pair-priority を継承
        - interaction graph を作って critical group を検出
        - group-priority successor を一般 DFS より前に生成
        - group-aware dedup を導入
    """
    ...
    def __init__(
        ...
        group_priority_successors_per_pattern: int = 2,
        max_group_order_patterns: int = 4,
    ) -> None:
        """初期化する。
        ...
        Args:
            ...
            group_priority_successors_per_pattern:
                group-priority パターンごとの生成上限。
            max_group_order_patterns:
                focus group に対して試す順序候補数上限。
        """
        ...
        self.group_priority_successors_per_pattern = (
            group_priority_successors_per_pattern
        )
        self.max_group_order_patterns = max_group_order_patterns
    ...
    def _solve_single_agent_fast_path(
        ...
        return LaCAMSolution(
            ...
            raw_status="solved",
            notes="orientation_lacam_v4_single_agent_fast_path",
        )
    ...
    def _low_level_expand(
        ...
        focus_group = self._select_focus_group(
            problem=problem,
            curr_config=curr_config,
        ...
        group_priority_successors: List[JointSuccessorCandidate] = []
        if focus_group is not None and len(focus_group) >= 3:
            group_priority_successors = self._generate_group_priority_successors(
                problem=problem,
                curr_config=curr_config,
                goal_config=goal_config,
                candidate_moves=candidate_moves,
        ...
        general_result = self._construct_joint_successors(
            ...
            robot_order_override=None,
        )

        merged = (
            list(pair_priority_successors)
            + list(group_priority_successors)
            + list(general_result.successors)
        )
        ...
        return LowLevelExpandResult(
            ...
            notes="pair_plus_group_plus_general",
        )
    ...
    def _select_focus_group(
        ...
        problem: LaCAMProblem,
        curr_config: Config,
        critical_groups: Sequence[Tuple[int, ...]],
        candidate_moves: Dict[int, List[CandidateMove]],
        goal_config: Config,
    ) -> Optional[Tuple[int, ...]]:
    ...
    def _generate_pair_priority_successors(
        ...
        for winner, loser in ((robot_i, robot_j), (robot_j, robot_i)):
            adjusted = self._build_pair_role_adjusted_candidate_moves(
                curr_config=curr_config,
                candidate_moves=candidate_moves,
                winner=winner,
                loser=loser,
            ...
            result = self._construct_joint_successors(
                ...
                robot_order_override=(winner, loser),
            )
    ...
    def _build_pair_role_adjusted_candidate_moves(
        self,
        curr_config: Config,
        candidate_moves: Dict[int, List[CandidateMove]],
        winner: int,
    ...
    def _generate_group_priority_successors(
        self,
        problem: LaCAMProblem,
        curr_config: Config,
        goal_config: Config,
        ...
        """focus group に対して group-priority successor を生成する。"""
        order_patterns = self._enumerate_group_order_patterns(
            problem=problem,
            curr_config=curr_config,
            goal_config=goal_config,
            candidate_moves=candidate_moves,
        ...
        for order_pattern in order_patterns:
            adjusted = self._build_group_role_adjusted_candidate_moves(
                curr_config=curr_config,
                candidate_moves=candidate_moves,
                ordered_group=order_pattern,
            )
            result = self._construct_joint_successors(
                problem=problem,
                curr_config=curr_config,
                candidate_moves=adjusted,
                next_time_step=next_time_step,
                max_successors=self.group_priority_successors_per_pattern,
                robot_order_override=order_pattern,
            )
            successors.extend(result.successors)
    ...
    def _enumerate_group_order_patterns(
        ...
        return patterns[: self.max_group_order_patterns]

    def _build_group_role_adjusted_candidate_moves(
        self,
        curr_config: Config,
        candidate_moves: Dict[int, List[CandidateMove]],
        ordered_group: Sequence[int],
    ) -> Dict[int, List[CandidateMove]]:
        """group order に応じて候補順を調整する。"""
        adjusted: Dict[int, List[CandidateMove]] = {}
        group_rank = {
            robot_id: rank for rank, robot_id in enumerate(ordered_group)
        }
        group_size = len(ordered_group)

        for robot_id, moves in candidate_moves.items():
            if robot_id not in group_rank:
                adjusted[robot_id] = list(moves)
                continue

            adjusted[robot_id] = self._sort_moves_for_group_rank(
                curr_state_id=curr_config[robot_id],
                moves=moves,
                rank=group_rank[robot_id],
                group_size=group_size,
            )

        return adjusted

    def _sort_moves_for_group_rank(
        self,
        curr_state_id: int,
        ...
        """group 内の順位に応じて候補順を調整する。"""
        if rank == 0:
            return self._sort_moves_for_advance(curr_state_id=curr_state_id, moves=moves)

        if rank == group_size - 1:
            return self._sort_moves_for_yield(curr_state_id=curr_state_id, moves=moves)

        def key_fn(move: CandidateMove) -> Tuple[int, float]:
            is_wait = move.next_state_id == curr_state_id
            adjusted_score = move.score + (-0.2 if is_wait else 0.0)
            return (0 if not is_wait else 1, adjusted_score)
    ...
    def _construct_joint_successors(
        ...
        next_time_step: int,
        max_successors: int,
        robot_order_override: Optional[Sequence[int]],
    ) -> LowLevelExpandResult:
        """複数の feasible joint successor を構築する。"""
        robot_order = self._decide_assignment_order(
            candidate_moves=candidate_moves,
            robot_order_override=robot_order_override,
        )
    ...
    def _decide_assignment_order(
        ...
        all_robot_ids = list(candidate_moves.keys())

        if robot_order_override is None:
        ...
        override_set = set(robot_order_override)
        remaining = [rid for rid in all_robot_ids if rid not in override_set]
        remaining.sort(
            key=lambda rid: (
                len(candidate_moves[rid]),
                candidate_moves[rid][0].score
        ...
        return list(robot_order_override) + remaining
    ...
    def _group_resolution_signature(
        self,
        problem: LaCAMProblem,
        curr_config: Config,
        next_config: Config,
        ...
        """focus group 内だけを見るシグネチャを返す。"""
        graph = problem.graph
        signature: List[Tuple[int, bool, str]] = []

        for robot_id in focus_group:
            edge = graph.edge_from_state_ids(
                curr_config[robot_id],
                next_config[robot_id],
            )
            changed = curr_config[robot_id] != next_config[robot_id]
            signature.append((robot_id, changed, edge.action_type.value))
    ...
    def _deduplicate_joint_successors(
        ...
        focus_group: Optional[Tuple[int, ...]],
    ) -> List[JointSuccessorCandidate]:
        """類似 successor を削減する。"""
        ...
        if focus_group is None:
            ...
            best_by_signature_group: Dict[
                Tuple[Tuple[int, bool, str], ...],
                JointSuccessorCandidate,
            ] = {}
            for successor in best_by_config.values():
                signature = self._group_resolution_signature(
                    problem=problem,
                    curr_config=curr_config,
                    next_config=successor.next_config,
                    focus_group=focus_group,
                )
                existing = best_by_signature_group.get(signature)
                if existing is None or successor.score < existing.score:
                    best_by_signature_group[signature] = successor
            deduped = list(best_by_signature_group.values())
