...
class BackendDiagnostics:
    """PIBT / low-level の診断ログ。"""

    low_level_calls: int = 0
    low_level_queue_expansions: int = 0
    partial_constraint_expansions: int = 0
    pibt_calls: int = 0
    pibt_displacements: int = 0
    pibt_failures: int = 0
    successful_priority_orders: int = 0
    first_blocking_conflict_type: str = "none"

    def to_note(self) -> str:
        """診断ログを note 文字列へ変換する。"""
        return (
            "diag:"
            f"low_level_calls={self.low_level_calls},"
            f"low_level_queue_expansions={self.low_level_queue_expansions},"
            f"partial_constraint_expansions={self.partial_constraint_expansions},"
            f"pibt_calls={self.pibt_calls},"
            f"pibt_displacements={self.pibt_displacements},"
            f"pibt_failures={self.pibt_failures},"
            f"successful_priority_orders={self.successful_priority_orders},"
            f"first_blocking_conflict_type={self.first_blocking_conflict_type}"
        )


class OrientationLaCAMBackendV5_1(LaCAMBackend):
    """姿勢付き MAPF 向け LaCAM 骨格 backend の v5.1。

    v5.1 の主眼:
        - v5 の PIBT low-level を維持
        - `_func_pibt()` の `goal_config` バグを修正
        - PIBT / low-level の診断ログを埋め込む
    """
    ...
    def solve(
        ...
        """LaCAM 骨格 v5.1 で問題を解く。"""
        del config
        self._diag = BackendDiagnostics()

        if problem.num_agents == 1:
            solution = self._solve_single_agent_fast_path(problem)
            return LaCAMSolution(
                solved=solution.solved,
                comp_time_sec=0.0,
                paths_by_agent=solution.paths_by_agent,
                raw_status=solution.raw_status,
                notes=solution.notes + " | " + self._diag.to_note(),
            )
        ...
        while open_heap:
            ...
            if self._is_goal_config(curr_config, goal_config):
                return LaCAMSolution(
                    ...
                    raw_status="solved",
                    notes="orientation_lacam_v5_1 | " + self._diag.to_note(),
                )
        ...
        return LaCAMSolution(
            ...
            notes="open list exhausted | " + self._diag.to_note(),
        )
    ...
    def _solve_single_agent_fast_path(
        ...
        return LaCAMSolution(
            ...
            raw_status="solved",
            notes="orientation_lacam_v5_1_single_agent_fast_path",
        )
    ...
    def _low_level_expand_with_pibt(
        ...
        for priority_order in priority_orders[: self.max_priority_orders]:
            ...
            while (
                ...
                low_expansions += 1
                self._diag.low_level_queue_expansions += 1
                ...
                if (
                    ...
                    self._diag.first_blocking_conflict_type = (
                        pibt_result.blocking_conflict.conflict_type
                    )
                ...
                self._diag.partial_constraint_expansions += len(children)
            ...
            if order_success:
                self._diag.successful_priority_orders += 1
    ...
    def _build_priority_order_candidates(
        ...
        if group is not None and len(group) >= 3:
            group_orders = self._enumerate_group_order_patterns(
                problem=problem,
                curr_config=curr_config,
                goal_config=goal_config,
                candidate_moves=candidate_moves,
    ...
    def _func_pibt(
        ...
        try:
            # 現在の実装では goal_config 自体は直接使わないが、
            # 将来の PIBT 専用 ranking 拡張に備えて引数は残す。
            _ = goal_config
            ...
            for move in context.candidate_moves[robot_id]:
                ...
                if occupant is not None and occupant != robot_id:
                    ...
                    self._diag.pibt_displacements += 1
    ...
    def _expand_partial_constraint_node(
        ...
        for move in candidate_moves[target_robot][
            : self.max_partial_constraints_per_expand
        ]:
            new_assignments = list(partial_node.assignments)
            new_assignments.append((target_robot, move.next_state_id))
            score = partial_node.score + move.score - 0.1 * partial_node.depth
            children.append(
    ...
    # ------------------------------------------------------------------
    # Missing methods restored in complete version
    # ------------------------------------------------------------------
        ...
        for _, robot_id, next_state_id in candidates[: self.max_branch_children]:
            ...
            children.append(
                self._make_child_with_added_constraint(
                    ...
                    goal_config=goal_config,
                    notes="branch_no_successor_v5_1",
                )
            )
